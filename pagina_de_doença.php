...
$sql = "select * from conteudodaspaginas  where cd_conteudo= '$id'";

$result=mysqli_query($mysqli,$sql);

while ($row = mysqli_fetch_array($result)) {
    $_SESSION['titulo_artigo']=$row['titulo'];
    $texto_artigo=$row['texto'];
}
?>
...
<link rel="stylesheet" href="css/style_paginas_doença.css">
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>

<div class="row justify-content-center">
    <div class='row col-9  shadow-lg p-3 mb-5 bg-body rounded fundo '>
        <h1 class='text-center'> <?php echo $_SESSION['titulo_artigo']; ?> </h1><hr><br><br>
        <div class="col-lg-12  col-md-10 col-sm-10 mt-sm-3 texto bg-white" >
            <p class=""><?php echo $texto_artigo; ?></p> <br><br>
        </div>
    </div>
...
<!-- Inicio dos cards -->
    <div class="row  justify-content-center">
        <div class="shadow-lg p-3 mb-5 bg-body rounded row col-9 text-white"  style="background-image: radial-gradient(circle at 50% 50%,#2f1632 0,#291030 25%,#21092e 50%,#1a002c 75%,#13002b 100%); font-family: 'Praise', cursive;"> <p class="text-center"><h1 class="text-center">Top 3 Publicações </h1></p></div><br>
...
<?php 
            ...
            $sql = "SELECT count(*) as total from tb_forum where cd_transtorno = '$id'";
            $result = mysqli_query($mysqli , $sql);
...
<div class="shadow-lg p-3 mb-5 bg-body rounded text-white row col-9 " style="background-image: radial-gradient(circle at 50% 50%,#2f1632 0,#291030 25%,#21092e 50%,#1a002c 75%,#13002b 100%);"> <p class="text-center"><h1 class="text-start">Geral </h1></p></div>
        
<?php  
    ini_set('display_errors', 0 ); error_reporting(0);
    $sql = "select * from tb_forum where cd_transtorno = '$id'  ORDER BY cd_post DESC";
...
?>                                                    
        <div class='row col-lg-9 col-md-8 col-sm-8 shadow-lg p-3 mb-5 bg-body rounded fundo mt-4 ' id="<?php echo $cd_post?>">
    ...
    <script> 
        ...
        setInterval(function (){
    $('#top3').load("config/fetch_top3.php?idpost=<?php echo $id ?>");
    }, 1500);
    </script>
    <style>
...
<?php 
        ...
        if($row['total'] == 0){
            echo'
             <div class="shadow-lg p-3 mb-5 bg-body rounded mb-3 col-9 p-4">
                <div class="card-header"><h1>Atenção!!</h1></div>
                    <div class="">
                        <p class="card-text p-3"> Nenhum Post encontrado!! (Provavelmente não foi postado nenhum artigo sobre o tema) </p>
                    </div>
